...
| Pi | `${PI_CODING_AGENT_SESSION_DIR:-~/.pi/agent/sessions}/**/*.jsonl` | Native extension |
...
| Pi | Native extension | `before_agent_start`, `agent_settled`, `input` | Recalls memory before a request, captures only fully settled turns, and supports `/memmy-resume` |
...
- **Automatic recall:** Claude Code, Codex, Pi, OpenCode, OpenClaw, and Hermes retrieve and inject relevant historical memory before normal requests run.
...
- **Source attribution:** captured turns carry `cursor`, `claude_code`, `codex`, `pi`, `opencode`, `openclaw`, or `hermes`, so you can filter and trace their origin.
...
| --- | --- | --- |
| `before_agent_start` | After prompt expansion, before the agent loop | Opens the shared Memmy session, starts the turn, and injects recalled context without blocking Pi if Memory is unavailable |
