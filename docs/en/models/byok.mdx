...
Set `providers.*.apiBase` to override a provider's default endpoint. For an unregistered OpenAI-compatible gateway, use the built-in `custom` provider:
...
Anthropic-compatible gateways can use `provider: anthropic` with `providers.anthropic.apiBase`.
