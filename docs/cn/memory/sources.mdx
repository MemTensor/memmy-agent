...
| Pi | `${PI_CODING_AGENT_SESSION_DIR:-~/.pi/agent/sessions}/**/*.jsonl` | 原生 Extension |
...
| Pi | 原生 Extension | `before_agent_start`、`agent_settled`、`input` | 请求前召回，只采集完全稳定的回合，并支持 `/memmy-resume` |
...
- **自动召回**：Claude Code、Codex、Pi、OpenCode、OpenClaw 和 Hermes 会在正常请求执行前检索相关记忆并注入上下文。
...
| 事件 | 时机 | 行为 |
| --- | --- | --- |
| `before_agent_start` | Prompt 展开后、Agent 循环前 | 打开共享 Memmy 会话、启动回合并注入召回上下文；Memory 不可用时不阻断 Pi |
...
1. 打开 **记忆管理 → 跨Agent接入**。
2. Cursor、Claude Code、Codex 点击 **安装 Hook**；Pi、OpenCode、OpenClaw、Hermes 点击 **安装插件**。
