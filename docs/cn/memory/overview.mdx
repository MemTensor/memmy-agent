...
workspace 能解析到项目时，`turn.start` 会在召回记忆前先放入独立的 `<memmy_project_context>` 区块。这里仅包含已确认的项目权威状态：当前目标及其约束、选中的工作项和下一步。Agent 来源与 profile 用于记录出处，不会切分项目，因此同一 workspace 中的 Codex、Pi 和其他已安装接入会收到同一份确认状态。Hook 与插件的具体行为见[Agent 来源与扫描](/cn/memory/sources)。
