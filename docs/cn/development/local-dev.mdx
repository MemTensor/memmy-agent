...
> `.env` 中 `MEMMY_CLOUD_SERVICE` 默认指向 `https://api.memmy.cn`（只填写域名，不要追加 `/api`，客户端会自动拼接接口路径），复制即连官方云服务——无需自建后端、无需自备 API Key。Windows 请在 Git Bash 中运行；脚本会把 `~/.local/bin` 加入 PATH。
