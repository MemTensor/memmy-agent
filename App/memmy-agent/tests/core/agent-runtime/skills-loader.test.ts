...
describe("SkillsLoader Computer History platform availability", () => {
  it.each(["win32", "linux"] as const)("hides builtin Computer History from all skill entry points on %s", (platform) => {
    ...
    writeSkill(builtin, "docx", { body: "# Office Helper" });
    const loader = new SkillsLoader(workspace, builtin);

    expect(loader.listSkills(false).map((skill) => skill.name)).toEqual(["browser-helper", "docx"]);
    expect(loader.listSkills(true).map((skill) => skill.name)).toEqual(["browser-helper", "docx"]);
    ...
    expect(loader.loadSkill("docx")).toContain("# Office Helper");
  });
