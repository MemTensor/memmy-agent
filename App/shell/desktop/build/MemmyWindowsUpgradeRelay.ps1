...
function Test-MemmyJsonObjectHasExactProperties([object]$Value, [string[]]$ExpectedNames) {
...
function Test-MemmyCanonicalUtcTimestamp([object]$Value) {
...
function Test-MemmyWorkspaceMigrationLedgerJson([object]$State) {
  if (-not (Test-MemmyJsonObjectHasExactProperties $State @('formatVersion', 'scope', 'applied'))) { return $false }
  ...
  foreach ($record in @($State.applied)) {
    ...
    if (-not (Test-MemmyJsonObjectHasExactProperties $record $expectedRecordNames)) { return $false }
    if ($record.id -isnot [string] -or -not [string]$record.id.Trim()) { return $false }
    if ($record.introducedIn -isnot [string]) { return $false }
    if (-not (Test-MemmyCanonicalUtcTimestamp $record.appliedAt)) { return $false }
    ...
    if ($formatVersion -eq 2) {
      ...
      if ($targetType -ceq 'agent-workspace') {
        if (-not (Test-MemmyJsonObjectHasExactProperties $target @('type'))) { return $false }
      }
      elseif ($targetType -ceq 'runtime-config' -or $targetType -ceq 'session-dag') {
        if (-not (Test-MemmyJsonObjectHasExactProperties $target @('type', 'key'))) { return $false }
...
function Test-MemmyDirectoryContainsOnlyValidWorkspaceMigrationLedger([string]$Path) {
  if (-not (Test-Path -LiteralPath $Path -PathType Container)) { return $false }
  try {
    ...
    return Test-MemmyWorkspaceMigrationLedgerJson $state
  } catch {
    return $false
  }
}
...
function Assert-MemmyRelocationTargetIsSafe([switch]$AllowPreparedMigrationTarget) {
  ...
  $targetRuntimeHasData = Test-MemmyDirectoryContainsData $targetRuntimeHomePath @('updates')
  $targetRuntimeHasOnlyValidWorkspaceMigrationLedger =
    Test-MemmyDirectoryContainsOnlyValidWorkspaceMigrationLedger $targetRuntimeHomePath
  if ($targetRuntimeHasData -and -not $targetRuntimeHasOnlyValidWorkspaceMigrationLedger) {
