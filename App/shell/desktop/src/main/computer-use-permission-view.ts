/** All dynamic values use textContent. The view has no remote resources. */
...
<p id="message" class="note" role="status" aria-live="polite">返回此窗口后会自动检测权限。</p>
<details><summary>遇到问题？</summary><p>请在系统设置中开启 Open Computer Use。若有同名记录或找不到程序，点击列表中的“+”，按 ⌘⇧G，粘贴下方路径并添加。完成后返回此窗口。</p><code id="helper-path"></code><button id="copyPath">复制程序路径</button></details>
...
bridge.subscribe(state=>{
  ...
  el('message').textContent=state.busy?'正在检测权限…':state.message || '返回此窗口后会自动检测权限。';
